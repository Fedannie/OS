...
void allocator_init() {
    ...
    size_t max_mem_size = ((memory_map[memory_map_size - 1].begin + memory_map[memory_map_size - 1].length));
    ...
    for (size_t i = 0; i < memory_map_size; ++i) {
        if (memory_map[i].type == 1) {
            if (memory_map[i].length >= boot_size) {
                boot_mem = va(memory_map[i].begin);
                memory_map[i].begin += boot_size;
    ...
    for (size_t i = 0; i < memory_map_size; ++i) {
        if (memory_map[i].type == 1) {
            while (memory_map[i].length >= PAGE_SIZE) {
                uint64_t start_addr = ((memory_map[i].begin + 1) / PAGE_SIZE) * PAGE_SIZE;
                if (start_addr + PAGE_SIZE > memory_map[i].begin + memory_map[i].length) {
                ...
                memory_map[i].begin = start_addr + PAGE_SIZE;
